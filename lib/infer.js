...
(function(mod) {
  ...
  AVal.prototype = extend(ANull, {
    ...
    getType: function(guess, backprop) {
      if (backprop !== false) {
        var bt = this.getBackPropagateType();
        if (/requirejs_/.test(this.origin)) console.error(this.origin);
        if (this.origin == 'requirejs_extend_this') {

        }
        if (bt) console.error('BT!');
        if (bt && !this.hasType(bt)) {
          this.addType(bt);
        }
      }
    ...
    getBackPropagateType: function(seen) {
      if (!seen) seen = [];
      if (seen.indexOf(this) != -1) return;
      seen.push(this);
      if (this.forward) for (var i = 0; i < this.forward.length; i++) {
        var fwd = this.forward[i];
        var type = fwd.getType(null, false);
        if (type && type.backPropagate) {
          return type;
        }
        type = fwd.getBackPropagateType(seen);
        if (type) return type;
      }
      return null;
    },
